...
export function TaskDetailSheet({
  ...
  const stream = task ? STREAM_MAP[task.streamId] : undefined;
  const status = task ? taskStatus(task) : "planned";
        ...
        {task ? (
          <>
            ...
            <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 pb-2">
              ...
              <div>
                <div className="mb-2 text-sm text-muted-foreground">주간 진행 (클릭하여 변경)</div>
            ...
            <SheetFooter>
              <p className="text-xs text-muted-foreground">
                변경은 이 브라우저에 자동 저장됩니다. 주간 업무 보고가 아니라 8–12월 전체 개발 트랙입니다.
              </p>
            </SheetFooter>
          </>
        ) : null}
